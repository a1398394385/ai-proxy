...
注意：方法内部的 `from .upstream_driver import UpstreamDriver`（`_forward_non_streaming` 和 `_forward_streaming` 中各一处）不在此 Task 处理——Task 2/3 会整体替换这两个方法，这些 import 自然消除。

- [ ] **Step 4: 验证编译报错暴露所有调用点**

```bash
python3 -c "from proxy.handler import ProxyHandler" 2>&1
```

预期：报错提示 `UpstreamDriver` 未定义（来自尚未重写的 `_forward_non_streaming` / `_forward_streaming` 方法体）。这些报错指向的就是 Task 2-3 需要替换的方法。
...
- [ ] **Step 1: 在 handler.py 的 import 块之后、类 ProxyHandler 定义之前添加路径映射常量**

在 `from .agent_detector import detect_subagent`（约第 45 行）之后，`class ProxyHandler`（约第 47 行）之前添加：
...
代码中使用的辅助函数均已在 handler.py 中导入或可通过 `from .transform import` 访问：
- `_format_sse_event()` — `proxy/sse_utils.py`，通过 transform 模块 re-export
- `_parse_sse_event()` — `proxy/transform_responses.py`，handler.py 第 44 行已直接导入
- `generate_response_id()` — `proxy/transform_responses.py`，通过 transform 模块 re-export
- `TransformRouter` 所有方法均为 `@classmethod`，无需实例化
...
Mock 路径选择说明：`handler.py` 通过 `from .common import _create_upstream_conn` 导入，在 handler 模块命名空间创建本地绑定。根据 `unittest.mock.patch` 的 "patch where used" 原则，需 patch `proxy.handler._create_upstream_conn`（与现有代码 patch `proxy.upstream_driver.OpenAI` 同理）。
...
预期：全部通过，数量约 523（比当前 531 少 8 个被删的 driver 测试）。
