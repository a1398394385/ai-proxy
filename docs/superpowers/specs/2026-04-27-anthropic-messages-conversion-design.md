...
| `top_p` | `top_p` | 直接透传 |
| `stop_sequences` | `stop` | 数组直接透传（Chat API 同时接受 string 和 array） |
...
**推理字段兼容**: OpenAI 上游的推理 delta 字段因提供商而异——OpenAI 原生用 `delta.reasoning_content`，OpenRouter/兼容层用 `delta.reasoning`。实现时同时检测两个字段（先查 `reasoning_content`，再查 `reasoning`），参考 cc-switch `streaming.rs` 的做法。
...
class AnthropicStreamState:
    ...
    current_tool_id: str = ""       # 当前 tool_use block 的 id
    current_tool_name: str = ""     # 当前 tool_use block 的 name
    pending_args: str = ""          # 延迟到达的 tool arguments（id/name 未到齐时缓冲）
    finish_reason: str = ""
...
**"late start" 处理**: 参考 cc-switch `streaming.rs` line 447-488，当 tool call 的参数 chunk 先于 `id` 和 `name` 到达时（early arguments），缓存到 `pending_args`，等 `id`/`name` 到齐后再发送 `content_block_start` + 缓冲的内容。
...
1. **单元测试** — `test/test_transform_anthropic.py`
   - `anthropic_to_chat`: 基本消息转换、system 字符串/数组、多模态图片、工具定义、tool_use/tool_result、thinking → reasoning_effort、cache_control 保留、o-series max_completion_tokens、空 messages、错误格式
