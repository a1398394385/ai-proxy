...
**`_format_sse_event` 与 Anthropic 事件兼容性**: 当前 `_format_sse_event` 会为 data 注入 `"type"` 字段，并对 `response.` 和 `output_item.` 前缀的事件做 `"response"` / `"item"` 键包裹。Anthropic 事件类型（`message_start`、`content_block_delta` 等）不匹配这些前缀，因此不会被包裹——正确。迁移到 `sse_utils.py` 时需明确约定：**如果 data 中已包含 `"type"` 字段，不重复注入**（Anthropic 事件在构建 data 时已写入 `"type"`），避免覆盖或字段重复。两种 SSE 模块共用同一函数，无需为 Anthropic 单独适配。
...
| `tool_result` (`tool_use_id, content`) | **独立** `{role:"tool", tool_call_id, content}` 消息 | 从原消息中分离；content 为数组时 `json.dumps` 序列化为字符串。若数组含复杂对象（如 `type:"image"`），取第一个 text 块内容并记录 warning |
...
**字符串格式兜底**: 虽然 Claude Code 的 tool_choice 始终为对象格式，但如果未来其他 Anthropic 客户端发送字符串：
...
**usage 发送时机**: 仅当 `finish_reason` 出现时的那条 `message_delta` 携带 `usage`（含 `output_tokens`、`cache_read_input_tokens` 等），中间的 delta chunk 不携带 usage。与 cc-switch `streaming.rs` 行为一致。
...
**UTF-8 安全缓冲**: `create_anthropic_sse_stream` 内部维护 UTF-8 remainder 缓冲区，处理多字节字符在 TCP chunk 边界被切分的情况。当前 `_forward_streaming`（proxy.py）使用 `resp.read(chunk_size)` 逐块读取，不保证 UTF-8 边界对齐——UTF-8 安全由流式转换器内部处理，与 `create_codex_sse_stream` 保持一致（如有需要，可后续在 proxy.py 读取层统一处理）。
...
| `delta.tool_calls[i].function.arguments` 为 null/缺失 | 跳过该 delta，不发 `input_json_delta`（首个 chunk 可能只带 id 和 name，arguments 尚未到达） |
| tool_result content 含复杂对象 | 取第一个 text 块内容，记录 warning |
